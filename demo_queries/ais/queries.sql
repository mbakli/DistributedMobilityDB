...
----------------------------------------------------------------------------------------------------------------------
--Q5): What is the trajectory and speed of all ships that spent more than 5 days to reach to the port of
--     Kalundborg in Sept 19?
----------------------------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------------------------
SELECT Ship_id,
       MIN(trajectory(Trip) <-> ST_transform(ST_setsrid(ST_MakePoint(10.945873, 56.447255), 4326), 25832)) AS MinDistance
FROM ships_tanker_30t
GROUP BY mmsi
ORDER BY mmsi;
