...
BEGIN
    ...
    ELSIF tiling.internaltype in ('sequence','sequenceset') THEN
        -- Previously tried tgeompoint_discseq(array_agg(...)), which was a
        -- silent no-op (replace() pattern never matched the table-prefixed
        -- column, and the function doesn't exist in this MobilityDB version
        -- anyway). atStbox(distCol, bbox) is the real clipping primitive,
        -- applied per row/tile pair, no aggregation needed. Verified: a trip
        -- spanning two tiles (2658 instants) clips to 1489 + 1171.
